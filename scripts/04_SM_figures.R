...
# looking at values (e.g. transpiration) for all treatments (includeing control)
# instead of just showing data of the difference from the control
...
g <- tot_transp %>%
  ...
  theme_classic() +
  labs(x = aridity_lab,
       caption = "showing data from simulations with ambient warming and loam soil")+
  theme(legend.position = 'top')

pdf("figures/soil_moisture/SM_across_lyrs_not-diffs_v1.pdf",
    width = 8, height = 6)

# * tot transpiration ----------------------------------------------------

g +
  geom_point(aes(y = TRANSP)) +
